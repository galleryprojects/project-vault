...
export default function ArchiveManager({ vaultStats, setVaultStats }: any) {
  ...
  const AddGate = ({ tier, label, isVideoGate = false }: any) => (
    <div className={`relative aspect-square border-2 border-dashed ${isVideoGate ? 'border-[#3B82F6] bg-[#3B82F6]/5' : 'border-[#3B82F6]/20 bg-black'} flex flex-col items-center justify-center transition-all hover:border-[#3B82F6] group`}>
      ...
      <span className={`text-[14px] font-black ${isVideoGate ? 'text-[#3B82F6]' : 'text-gray-700'} group-hover:scale-125 transition-transform`}>+</span>
      <span className={`text-[8px] font-black uppercase mt-1 ${isVideoGate ? 'text-[#3B82F6]' : 'text-gray-500'}`}>
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] gap-8 animate-in fade-in duration-500">
      ...
      <div className="w-1/4 flex flex-col gap-4">
        <input type="text" placeholder="SEARCH_VAULTS..." className="bg-black border border-[#3B82F6]/20 p-3 text-[10px] text-white outline-none focus:border-[#3B82F6] uppercase font-black" onChange={(e) => setSearchQuery(e.target.value)} />
        <div className="flex-1 overflow-y-auto border border-[#3B82F6]/10 space-y-1 scrollbar-hide">
          {vaultStats.filter((v: any) => v.id.includes(searchQuery.toLowerCase())).map((v: any) => {
            ...
            return (
              <button key={v.id} onClick={() => setSelectedCollection(v.id)} className={`w-full text-left p-4 text-[11px] font-black uppercase transition-all ${selectedCollection === v.id ? 'bg-[#3B82F6] text-black shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
      ...
      <div className="flex-1 flex flex-col gap-6">
          ...
          {!selectedCollection ? (
            ...
            <>
              {[1, 2, 3, 99].map((tierNum) => {
                ...
                return (
                  <div key={tierNum} className="border border-[#3B82F6]/10 bg-black/10 overflow-hidden">
                    <button 
                      onClick={() => toggleTier(tierNum)}
                      className="w-full flex justify-between items-center p-4 bg-black/60 hover:bg-[#3B82F6]/10 transition-colors border-b border-[#3B82F6]/5"
                    >
                      <span className="text-[10px] font-black text-white tracking-[0.2em]">
                        // {tierName} <span className="text-[#3B82F6] ml-2">[{assets.length} ASSETS]</span>
                      </span>
                      <span className="text-[#3B82F6] font-black">{isOpen ? '−' : '+'}</span>
                    </button>
                        ...
                        {assets.map((asset, index) => {
                          ...
                          return (
                            <div key={asset.id} className="flex flex-col gap-3 group animate-in fade-in duration-500">
                              <div className={`media-grid-item aspect-square border transition-all ${asset.display_order === 0 ? 'border-[#3B82F6] shadow-[0_0_20px_rgba(59,130,246,0.3)]' : 'border-[#3B82F6]/10'} bg-black relative overflow-hidden`}>
                                <OptimizedMedia 
                                  ...
                                  priority={index < 3}
                                />
                                <div className="absolute top-2 left-2 px-2 py-1 bg-black/80 border border-[#3B82F6]/30 text-[7px] font-black text-[#3B82F6] z-10 uppercase">
                              ...
                              <div className="flex flex-col gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                {isVid && (
                                  <div className="space-y-2 mb-2 bg-[#3B82F6]/5 p-2 border border-[#3B82F6]/10 rounded">
                                    <div className="flex justify-between text-[7px] font-black text-[#3B82F6] uppercase">
                                      <span>Scrub: {currentStart}s</span>
                                      <span>${currentPrice}</span>
                                    </div>
                                    <input type="range" min="0" max="60" value={currentStart} onChange={(e) => handleUpdateAssetLocal(asset.id, { start_time: parseInt(e.target.value) })} className="w-full h-1 bg-[#3B82F6]/20 appearance-none cursor-pointer accent-[#3B82F6]" />
                                    <div className="flex items-center bg-black border border-[#3B82F6]/20 px-1 py-0.5">
                                      <span className="text-[8px] text-[#3B82F6] mr-1">$</span>
                                ...
                                {!isVid && (
                                  <button onClick={() => setMediaAsCover(selectedCollection, asset.id).then(() => getCollectionMedia(selectedCollection).then(setCollectionAssets))} className={`py-2 text-[8px] font-black uppercase transition-all ${asset.display_order === 0 ? 'bg-[#3B82F6] text-black' : 'bg-white/5 text-gray-400 hover:bg-white hover:text-black'}`}>{asset.display_order === 0 ? 'PRIMARY_LOCKED' : 'SET_AS_COVER'}</button>
                                )}
                                <div className="relative">
                                  <button className="w-full py-2 border border-[#3B82F6]/30 text-[#3B82F6] text-[8px] font-black uppercase hover:border-[#3B82F6]">SWAP_FILE</button>
        ...
        {/* 3. FOOTER */}
        {selectedCollection && (
          <div className="bg-black border border-[#3B82F6]/20 p-6 flex items-end gap-6 shadow-2xl relative">
            <div className="flex-1">
              ...
              <input readOnly type="text" value={selectedCollection} className="w-full bg-[#0a0a0a] border border-[#3B82F6]/10 p-3 text-gray-500 text-xs font-bold outline-none cursor-not-allowed uppercase" />
            </div>
            <div className="flex gap-4">
              ...
              <button onClick={handleGlobalSync} className={`bg-[#3B82F6] text-black px-12 py-3 text-[10px] font-black uppercase hover:bg-white transition-all ${uploadingTier === 0 ? 'animate-pulse' : ''}`}>{uploadingTier === 0 ? '[ SYNCING... ]' : '[ SAVE_CHANGES ]'}</button>
            </div>
          </div>
        )}
      </div>
